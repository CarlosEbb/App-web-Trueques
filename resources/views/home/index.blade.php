            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    @forelse(\App\Chat::orderBy("created_at", "desc")->where('user_id', Auth::user()->id)->orwhere('user_comprador_id', Auth::user()->id)->get()->groupBy('event', 'producto_id') as $chats)
                        ...
                        ?>
                        <a href="{{route('chat')}}?p={{$chats->first()->producto_id}}&v={{$chats->first()->user_id}}&c={{$chats->first()->user_comprador_id}}" @if($chats->first()->event == 'chat-event-'.$_GET["p"].'-'.$_GET["v"].'-'.$_GET["c"]) style="background: #f1f1f1;" @endif>
                            {{-- aqui --}}
                            <div class="card-body d-flex align-items-center border-bottom p-3 w-100" style="position: absolute;">
