...
@section('content')
    ...
    <div class="container" style="min-height: 65vh;">
      <div class="row">
        ...
        @forelse(\App\Chat::orderBy("created_at", "desc")->where('user_id', Auth::user()->id)->orwhere('user_comprador_id', Auth::user()->id)->get()->groupBy('event', 'producto_id') as $desclosado)
                <?php
                  $chat = $desclosado->first();
    
                    if(Auth::user()->id == $chat->user_id){
                        $toChat = $chat->user_comprador_id;
                    }else{
                        $toChat = $chat->user_id;
                    }
                ?>
              ...
              <div class="row no-gutters">
                ...
                <div class="col-md-10">
                  ...
                  <div class="card-footer py-0 bg-transparent d-flex justify-content-end">
                    <a class="btn btn-outline-info mx-2 mt-2 rounded-pill" href="{{route('chat')}}?p={{$chat->producto_id}}&v={{$chat->user_id}}&c={{$chat->user_comprador_id}}">Ir al chat</a>
