          ...
          @forelse(App\Models\ProductoFavorito::where('user_id', Auth::user()->id)->get() as $favorito)
            <li class="media media-style">
              ...
              <div class="media-body">
                <a href="{{route('productos.show', $favorito->producto->id)}}"><h5 class="mt-0 mb-0 d-inline-block" style="width: 90%;">{{$favorito->producto->nombre}}</h5></a>
                <a class="btn-rounded btn-rounded-chat tooltips d-inline-block" @Auth href="{{route('chat')}}?p={{$favorito->producto_id}}&v={{\App\Models\Producto::find($favorito->producto_id)->user_id}}&c={{Auth::user()->id}}" @else href="{{route('login')}}" @endauth>
